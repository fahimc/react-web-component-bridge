...
function createContext(defaultValue) {
  ...
  context.Provider = function Provider(props) {
    return {
      ...
      props: { context, value: props.value },
...
const Children = {
  ...
  map(children, mapper) {
    return normalizeChildren(children).map(mapper);
  },
  forEach(children, mapper) {
    normalizeChildren(children).forEach(mapper);
  },
  only(children) {
...
function createPortal(children, target) {
  return { type: Portal, props: { target }, children: [children] };
}
  ...
  class CompiledReactElement extends HTMLElement {
    ...
    __rwcbRender() {
      ...
      try {
        ...
        const vnode = Component?.__rwcbForwardRef
          ? Component.render(componentProps, this.__rwcbPublicRef)
    ...
    __rwcbReadContext(context) {
      return this.__rwcbContextValues.has(context)
        ? this.__rwcbContextValues.get(context)
...
function renderVNode(vnode) {
  ...
  if (vnode.type === Portal) {
    const target =
      typeof vnode.props.target === "function" ? vnode.props.target() : vnode.props.target;
  ...
  if (vnode.type?.__rwcbForwardRef)
    return renderVNode(
      vnode.type.render({ ...vnode.props, children: vnode.children }, createRef())
    );
