...
function ChakraUiApp(props) {
    ...
    const [selectedName, setSelectedName] = useState("Dialog");
    const [dialogOpen, setDialogOpen] = useState(false);
    const context = useMemo(() => ({ accent, mode }), [accent, mode]);
    const selected = useMemo(() => registry.find((item) => item.name === selectedName) ?? registry[0], [selectedName]);
    const inspect = (name) => {
            ...
            h("main", { className: "chakra-grid" },
                ...
                h(Card, { title: "Button recipe" },
                    h("div", { className: "chakra-actions" },
                        h(Button, { onClick: () => props.onAction?.({ name: "Solid button recipe" }) }, "Solid"),
                        h(Button, { variant: "subtle", onClick: () => props.onAction?.({ name: "Subtle button recipe" }) }, "Subtle"),
                        h(Button, { variant: "outline", onClick: () => props.onAction?.({ name: "Outline button recipe" }) }, "Outline"))),
                h(Card, { title: "Composition registry" },
                    h("div", { className: "chakra-table" }, registry.map((item) => (h("div", { className: selectedName === item.name ? "selected" : "" },
                        h("strong", null, item.name),
                        h("span", null, activeTab),
                        h(Button, { size: "sm", variant: "outline", onClick: () => inspect(item.name) }, "Inspect")))))),
                h(Card, { title: `${selected.name} inspector` },
                    h("div", { className: "chakra-inspector" },
                        h("dl", null,
                            h("div", null,
                                h("dt", null, "Status"),
                                h("dd", null, selected.status)),
                            h("div", null,
                                h("dt", null, "Exports"),
                                h("dd", null, selected.exports))),
                        h("p", null, selected.notes),
                        selected.name === "Dialog" ? (h(Button, { variant: "outline", onClick: () => setDialogOpen(true) }, "Open dialog preview")) : (h(Button, { variant: "subtle", onClick: () => props.onAction?.({ name: selected.name }) }, "Run preview")))),
                dialogOpen ? (h("section", { className: "chakra-dialog", role: "dialog", "aria-modal": "true" },
                    h("div", { className: "chakra-dialog-panel" },
                        h("span", null, "Composition preview"),
                        h("h3", null, "Dialog composition"),
                        h("p", null, "The compiled component is managing open state, close actions, and nested composition content without React in the browser bundle."),
                        h("div", { className: "chakra-actions" },
                            h(Button, { size: "sm", onClick: () => props.onAction?.({ name: "Confirm Dialog" }) }, "Confirm"),
                            h(Button, { size: "sm", variant: "outline", onClick: () => setDialogOpen(false) }, "Close"))))) : null,
